...
class SendPingThr(threading.Thread):
    ...
    参数：
        ...
        timeout     -- 设置发送超时
    '''
    #__init__用于初始化类对象。当创建一个SendPingThr的新对象时，它首先调用threading.Thread.__init__(self)，然后定义接下来的五个属性
    ...
    def run(self):
        ...
        for ip in self.ipPool:
            try:
            #发送 UDP 数据，将数据发送到套接字，address 是形式为（ipaddr，port）的元组，指定远程地址。返回值是发送的字节数。构造的icmp报文
